...
#define MODBUS_MIN_MSG_LENGTH 4
...
void B1_CapacitanceTemperatureMeasure_Poll()
{
    ...
    case 1:
    {
        ...
        if (readLen >= MODBUS_MIN_MSG_LENGTH)
        {
            byte_t *dataBuffer = g_AppMainInfo.buffer;
            // Check CRC
            if (CRC16_Modbus(dataBuffer, readLen) == 0)
            {
                // Check function code and slaver address
                if (dataBuffer[0] == request_cmd[0] && dataBuffer[1] == request_cmd[1])
                {
        ...
        if (g_pSysInfo->capTempSensorTransmitConnect == true && !parseSuccess)
        {
