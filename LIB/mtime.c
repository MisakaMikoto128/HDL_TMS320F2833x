...
/* 每个月的天数 */
const byte_t g_day_per_mon[MONTH_PER_YEAR] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
 ...
 *     闰年返回1，否则返回0
 */
byte_t applib_dt_is_leap_year(uint16_t year)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *
 */
byte_t applib_dt_last_day_of_mon(byte_t month, uint16_t year)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
unsigned int mtime_2_unix_sec(mtime_t *currTime)
{
    ...
    {
        no_of_days += applib_dt_last_day_of_mon((byte_t)i, currTime->nYear);
    }
